...
Render::Render(const int w, const int h, const int bpp) : w(w), h(h), bpp(bpp), data(w*h*bpp, 0) {}

bool Render::read_tga_file(const std::string filename) {
    std::ifstream in;
...
bool Render::load_rle_data(std::ifstream &in) {
...
bool Render::write_tga_file(const std::string filename, const bool vflip, const bool rle) const {
...
bool Render::unload_rle_data(std::ofstream &out) const {
...
PixelColor Render::get(const int x, const int y) const {
    if (!data.size() || x<0 || y<0 || x>=w || y>=h)
        return {};
    return PixelColor(data.data()+(x+y*w)*bpp, bpp);
}

void Render::set(int x, int y, const PixelColor &c) {
    if (!data.size() || x<0 || y<0 || x>=w || y>=h) return;
    memcpy(data.data()+(x+y*w)*bpp, c.bgra, bpp);
}

void Render::flip_horizontally() {
    int half = w>>1;
